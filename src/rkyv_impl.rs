...
mod tests {
    ...
    fn serde_roundtrip() {
        ...
        let bytes = rkyv::to_bytes::<_, 1024>(&constants).expect("failed to serialize constants");
        // SAFETY:
        // - The byte slice represents an archived object
        // - The root of the object is stored at the end of the slice
        let deserialized = unsafe {
            rkyv::from_bytes_unchecked::<PoseidonConstants<S1, U2>>(&bytes)
                .expect("failed to deserialize constants")
        };
    ...
    fn serde_hash_blstrs() {
        ...
        // - The root of the object is stored at the end of the slice
        let deserialized = unsafe {
            rkyv::from_bytes_unchecked::<PoseidonConstants<Fr, U2>>(&bytes)
                .expect("failed to deserialize constants")
        };
        ...
        let mut h2 = Poseidon::<Fr, U2>::new_with_preimage(&preimage, &deserialized);
    ...
    fn serde_hash_pallas() {
        ...
        let mut h2 = Poseidon::<S1, U2>::new_with_preimage(&preimage, &deserialized);
